...
// Define the mapping outside the component for better performance
...
export default function CampaignStatusPage() {
  ...
  // id will be fetched from the URL dynamic segment [id]
          ...
          <div className="space-y-3">
            {platforms.map((platform: any) => (
              <div 
                ...
                className={`border rounded-lg p-4 ${PLATFORM_COLORS[platform.platformType] || 'bg-gray-50'}`}
              >
                <div className="flex justify-between items-center">
                  ...
                  <span className={`font-bold ${STATUS_COLORS[platform.status] || 'text-gray-500'}`}>
                    {(platform.status || 'unknown').toUpperCase()}
                  </span>
                </div>
              </div>
            ))}
